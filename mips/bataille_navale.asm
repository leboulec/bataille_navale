...
tire_debutant:							# Fonction tirant une torpille dans la case de coordonnées demandées à l'utilisateur en mode débutant
			...
			bgtz $t3, tire_debutant_bateau_present	# Si un bateau est présent sur cette case
										# Sinon il n'y a pas de bateau
			ori $t4, $zero, -1
			beq $t3, $t4, tire_debutant_deja_touche	# Si on a déjà touché cette case
			ori $t4, $zero, -2
			sw $t4, 0($s2)				# On met -2 dans la case
			la $a0, plouf
			ori $v0, $zero, 4
...
tire_debutant_bateau_proxi:
			la $a0, proxi
			ori $v0, $zero, 4
			syscall						# Affichage de pas loin!

			j tire_debutant_bateau_suite

tire_debutant_deja_touche:
			ori $t4, $zero, -1
			sw $t4, 0($s2)				# On met -1 dans la case
			la $a0, deja_touche
			ori $v0, $zero, 4
...
tire_debutant_bateau_present:
			...
			la $a0, touche
			beq $t5, $zero, tire_debutant_coule
			j tire_debutant_coule_suite
tire_debutant_coule:
			la $a0, coule
			lw $a1, 8($sp)
			lw $t6, 0($a1)
